...
async function onRequest3(context) {
  ...
  if (request.method !== "POST") {
    return json({ error: "M\xE9todo n\xE3o permitido. Use POST." }, 405);
  }
  const apiKey = env.CLAUDE_API_KEY;
  if (!apiKey) {
    return json({ error: "CLAUDE_API_KEY n\xE3o configurada nas vari\xE1veis de ambiente do Cloudflare." }, 500);
  }
  try {
    const body = await request.json();
...
async function onRequest4(context) {
  ...
  if (!apiKey) {
    return json2({ error: "GOOGLE_API_KEY n\xE3o configurada nas vari\xE1veis de ambiente do Cloudflare." }, 500);
  }
  if (!endpoint) {
    return json2({ error: 'Par\xE2metro "endpoint" obrigat\xF3rio.' }, 400);
  }
  try {
    let googleUrl;
    ...
    if (endpoint === "textsearch") {
      ...
      return json2({ error: `Endpoint desconhecido: ${endpoint}` }, 400);
    }
    const response = await fetch(googleUrl);
    const data = await response.json();
    return new Response(JSON.stringify(data), {
      status: response.status,
      headers: { ...CORS2, "Content-Type": "application/json", "Cache-Control": "public, max-age=300" }
    });
  } catch (err) {
    return json2({ error: "Erro interno: " + err.message }, 500);
  }
}
...
// ../.wrangler/tmp/pages-WMBzcR/functionsRoutes-0.8590556125234341.mjs
var routes = [
  {
    routePath: "/api/ai",
    mountPath: "/api",
