...
function goPage(name) {
  ...
  const titles = { setup:'Início', dashboard:'Dashboard', checklist:'Checklist', reviews:'Reputação & IA', competitors:'Concorrentes', heatmap:'Mapa de Calor', history:'Antes x Depois', proposal:'Proposta', manage:'Clientes', settings:'Config' };
...
function renderChecklist(biz, score) {
  ...
  const checks = [
    ...
    { l:'Galeria de fotos (Rigoroso: ≥30)', ok:p>=30, w:p>=10&&p<30, v:p+' foto'+(p!==1?'s':''), tip:'Perfis líderes na região possuem atualizações constantes de fotos (>30).' },
    ...
    { l:'Atualização de Ofertas e Google Posts', ok:false, w:true, v:'Requer verificação manual', tip:'O Google prioriza negócios com postagens nos últimos 7 dias.' },
    { l:'Adição recente de Vídeos', ok:false, w:true, v:'Requer verificação manual', tip:'Perfis que recebem upload de vídeos têm engajamento muito maior.' },
    { l:'Interação/Resposta a avaliações', ok:false, w:true, v:'Requer verificação manual', tip:'É obrigatório responder a TODOS os reviews (positivos e negativos).' }
  ];
...
async function loadCompetitors() {
  ...
  try {
    ...
    const nearData = await nearRes.json();
    if (nearData.results) {
      APP.competitors = nearData.results.filter(r => r.place_id !== biz.place_id).slice(0, 8).map(r => ({
        name: r.name, rating: r.rating || 0, total: r.user_ratings_total || 0,
...
async function generateHeatmap() {
  ...
  await Promise.all(gridPoints.map(async (pt, i) => {
    try {
      ...
      const data = await res.json();
      if (data.results) {
        const sorted = data.results.sort((a,b) => (b.rating||0) - (a.rating||0)); // Maps sort logic approx
        let rank = sorted.findIndex(r => r.place_id === biz.place_id);
  ...
  resultsGrid.forEach(rank => {
    const val = parseInt(rank) || 99;
    let bg = val <= 3 ? '#1D9E75' : val <= 10 ? '#BA7517' : '#E24B4A';
    if (rank === 'N/A' || rank === 'Erro') bg = '#999';
    html += `<div style="aspect-ratio:1;display:flex;align-items:center;justify-content:center;background:${bg};color:#fff;font-size:24px;font-weight:700;border-radius:50%;box-shadow:0 4px 10px rgba(0,0,0,0.15);border:3px solid #fff">${rank}</div>`;
  });

  html += `</div>
    <div style="margin-top:20px;background:var(--red-light);color:#791F1F;padding:14px;border-radius:8px;font-size:13px">
      <strong>Diagnóstico do Heatmap:</strong> Se há muitos pontos vermelhos, o negócio está "invisível" para clientes que estão a apenas 1km de distância. Estratégias de Check-ins, resposta rápida a reviews e postagens semanais ajudam a expandir esse raio verde!
    </div>
  `;
  el.innerHTML = html;
}
